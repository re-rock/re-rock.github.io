...
この例では、個々のチケットを表示できるようにルートに名前を追加しているため、ルートの名前を指定するだけでチケットの正しいURLをかんたんに作成できます。、
コードは次のようするだけです。
...
テンプレートが使用されている場所を確認するためにテンプレートをgrepする必要はありません。頻繁に使用するリンクの場合、この方法をオススメします。

## Where Next?

今回は、独自のシンプルアプリケーションをセットアップする方法を説明しました。これによってアプリ作成をすぐに始められ、例を参考にしながら素晴らしいものを作成してください。
